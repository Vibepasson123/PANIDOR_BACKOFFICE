 ...
 public function posView($id)
{
  ...
  /*  $locations= Mloaction::where('mpos_id','=',$id)->get();
    ...
    $discountP = product::whereHas('campaign_Product',function($query) use($id) {$query->where('mpos_id','=',$id); })->whereHas('campaign_Product', function($q){
        $q->where('end_date','<' ,Carbon::today());})->get();


     $camp2    =  MposCamp::where('mpos_id','=',$id)->get();
     $campaign =  Camp::whereHas('camp_mpos' ,function($query)  use($id) {$query->where('mpos_id','=',$id);} )->get();


     return view('mViews.mpos',compact('userMap',
       'locations',
       'S_tab',
       'newtab',
       'long',
