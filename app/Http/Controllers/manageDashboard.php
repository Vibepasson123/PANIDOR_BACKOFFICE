...
class manageDashboard extends Controller
{
   ...
   public function getHome(Request $request )
   {
     ...
     $offerProduct = product::whereHas('campaign_Product',function($query){ $query->where('status','=','1');})->get();
...
/*public function getHome(Request $request )
   ...
   // $reviews = reviews::where('created_at','<',Carbon::today()->subWeek())->get();
 $test2 = $test->groupBy('mpos_id');
 $reviews= $test2.'client_id'; 
